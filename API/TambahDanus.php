...
if ($sql->num_rows == 0) {
    ...
    if (move_uploaded_file($file['photo']['tmp_name'], $folderUpload . "/" . $nama_foto)) {
        $sql = "INSERT INTO `makanan`(`id_user`, `id_makanan`, `nama_makanan`, `deskripsi_makanan`, `foto_makanan`, `stok_harian`, `harga_satuan`, `nama_lengkap`, `alamat`, `no_telp`) VALUES ('$id_user','','$nama_makanan','$deskripsi_makanan','$nama_foto','$stok_harian','$harga_satuan','$nama_lengkap','$alamat','$no_telp')";
        $info = array();
        $info['sql'] = $sql;
